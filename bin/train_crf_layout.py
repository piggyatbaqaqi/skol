#!/usr/bin/env python3
"""Train the v4 Pass-1 layout CRF.

Walks ``skol_training_v2_no_golden`` (or any DB given via
``--source-db``), reads each doc's pre-computed v4 attachments
(``article.spans.v4.json`` + ``article.page-headers.json``), builds
per-line features via Step 2's ``build_line_features`` + per-line
labels via Step 3.C's ``build_label_sequence``, fits the CRF across
...
Usage::
    ...
    bin/train_crf_layout --source-db skol_training_v2_no_golden \\
...
from skol_classifier.v4.labels import build_label_sequence  # noqa: E402
...
def main() -> int:
    ...
    if verbosity >= 1:
        print(f'train_crf_layout — db={db_name} device={device}')
