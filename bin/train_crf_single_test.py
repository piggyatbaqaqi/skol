"""Tests for bin/train_crf_single.py — v4 layout CRF trainer."""
...
from train_crf_single import (  # type: ignore[import]  # noqa: E402
...
class TestInverseFrequencyWeights(unittest.TestCase):
    ...
    def test_balanced_corpus_yields_uniform_weights(self):
        # Equal counts across all 19 labels → all weights == 1.0.
        counts = np.array([100] * 19, dtype=np.float64)
        w = inverse_frequency_weights(counts)
        self.assertEqual(w.shape, (19,))
        self.assertTrue(np.allclose(w, 1.0))

    def test_rare_class_gets_higher_weight(self):
        # Misc-exposition dominates; Page-header rare.
        counts = np.zeros(19, dtype=np.float64)
...
class TestTrainingLoopIntegration(unittest.TestCase):
    ...
    def test_trains_and_saves(self):
        counts = train_one_run(
            ...
            device='cpu',
            redis_key='skol:classifier:model:v4_single_hand',
            meta_key='skol:classifier:model:v4_single_hand:meta',
            dry_run=False,
        )
        ...
        self.assertIn('skol:classifier:model:v4_single_hand',
                      self.redis.set_calls)
        self.assertIn('skol:classifier:model:v4_single_hand:meta',
                      self.redis.set_calls)
    ...
    def test_skip_existing_short_circuits(self):
        ...
        self.redis.store[b'skol:classifier:model:v4_single_hand'] = b'existing'
        self.redis.store[b'skol:classifier:model:v4_single_hand:meta'] = b'{}'
        counts = train_one_run(
            self.db, self.redis,
            doc_ids=['d1', 'd2'],
            sbert_lookup=self._zero_sbert,
