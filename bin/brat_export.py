...
def render_annotation_conf(entity_types: List[str]) -> str:
    ...
    Minimum viable conf: one ``[entities]`` section listing every
    type.  Without this, brat refuses to open the collection.
    No ``[relations]`` / ``[events]`` / ``[attributes]`` — Phase 1
    only uses entities (T-lines).

    The header comment captures provenance so the operator knows
    which entity types came from which export run.
    """
    if not entity_types:
        # Brat does open empty configs but the operator can't
        # annotate anything; emit a placeholder so the file
        # exists and the operator can extend it manually.
        return (
            '# brat annotation config — generated by '
            'bin/brat_export\n'
            '# (no entity types found in source DB)\n'
            '\n[entities]\n'
        )
    lines = [
