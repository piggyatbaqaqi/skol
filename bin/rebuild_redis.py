...
def list_existing_keys(redis_client, verbosity: int = 1) -> dict:
    ...
    for component, config in COMPONENTS.items():
        ...
        for pattern in config['keys']:
            ...
            # count=100000 keeps the per-pattern iteration fast on
            # large keyspaces — default of 10 produces ~1.4M round trips
            # on a 14.5M-key DB; 100000 cuts that to ~145.
            keys = redis_client.scan_iter(match=pattern, count=100000)
