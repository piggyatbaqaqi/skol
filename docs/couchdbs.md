...
- **`features_hand`** — one doc per reviewer-accepted annotation
  ...
  (`kept` or `added`).  Bootstrap-provenance fields (`model`,
  `created_at`) are preserved on `kept` annotations; `null` on
  reviewer-added annotations.  Same `_id` scheme as the candidate
