"""Tests for skol_classifier/v4/crf_treatment.py — v4 Pass-2 CRF model.

Mirrors crf_layout_test.py 1-for-1; the architecture is the same and
the contracts are the same, only the label vocab differs.
"""
...
class TestLabelSpace(unittest.TestCase):
    ...
    def test_treatment_labels_count(self):
        self.assertEqual(len(TREATMENT_LABELS), 12)
        self.assertEqual(N_LABELS, 12)

    def test_treatment_labels_contents(self):
        expected = {
            'Nomenclature', 'Description', 'Diagnosis', 'Etymology',
            'Materials-examined', 'Materials-and-methods',
            'Type-designation', 'Biology', 'Phylogeny',
            'New-combinations', 'Notes', 'Misc-exposition',
        }
        self.assertEqual(set(TREATMENT_LABELS), expected)
    ...
    def test_misc_exposition_index_consistency(self):
        self.assertEqual(
            TREATMENT_LABELS[MISC_EXPOSITION_INDEX], 'Misc-exposition',
        )
...
class TestConstruction(unittest.TestCase):
    ...
    def test_default_shapes(self):
        ...
        self.assertEqual(model.emission.out_features, 12)
        self.assertEqual(model.crf.num_tags, 12)

    def test_custom_dims(self):
        model = TreatmentCRF(feature_dim=64, n_labels=5)
        self.assertEqual(model.emission.in_features, 64)
        self.assertEqual(model.emission.out_features, 5)
        self.assertEqual(model.crf.num_tags, 5)
...
class TestForwardAndDecode(unittest.TestCase):
    ...
    def _batch(self, B=2, T=5, D=791, n_labels=12):
...
class TestSyntheticConvergence(unittest.TestCase):
    """Mirrors the Pass-1 test: identity-feature task converges to
    exact decoding in a handful of Adam steps."""
    ...
    def test_can_learn_id_function(self):
        ...
        model = TreatmentCRF(
            feature_dim=n_labels, n_labels=n_labels,
        )
        with torch.no_grad():
...
class TestSaveLoadRoundTrip(unittest.TestCase):
    ...
    def test_serialize_deserialize_preserves_weights(self):
        torch.manual_seed(7)
        model = TreatmentCRF(feature_dim=16, n_labels=4)
