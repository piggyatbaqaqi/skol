#!/usr/bin/env python3
"""
Test program for MdpiIngestor.

This program tests the metadata extraction from MDPI journal pages
without actually ingesting into the database. It displays extracted metadata
from the index page, volume pages, issue pages, and articles.
"""
...
def print_article(article, index):
    ...
    # Print each field
    for key in ['title', 'authors', 'volume', 'number',
                'doi', 'section', 'url', 'itemtype']:
        if key in article:
            value = article[key]
...
def test_index_page(ingestor):
    ...
    Args:
        ingestor: MdpiIngestor instance
...
def main():
    """Main test function."""
    print("\nMDPI Ingestor Test Program")
    print("Testing Journal of Fungi")
    print_separator('=')
    ...
    robot_parser.set_url('https://www.mdpi.com/robots.txt')
    robot_parser.read()

    # Create ingestor with verbose output
    ingestor = MdpiIngestor(
        db=mock_db,
    ...
    if articles:
        save_to_json(articles, 'mdpi_test.json')

    # Test 3: Complete workflow (single issue)
    print("\n\n" + "=" * 80)
    print("TEST 3: Complete Workflow (Index -> Volume -> Issue -> Articles)")
    print("=" * 80)

    final_articles = test_complete_workflow(ingestor, max_issues=1)

    if final_articles:
        save_to_json(final_articles, 'mdpi_complete_test.json')
